...
const EventForm = ({ userId, type,event,eventId }: EventFormProps) => {
   ...
   async function onSubmit(values: z.infer<typeof eventformSchema>) {
        ...
        if(type === 'Update'){
            ...
            try {
                const updateEventFrom = await updateEvent({
                    userId,
                ...
                if(updateEventFrom){
                    form.reset();
                    router.push(`/events/${updateEventFrom._id}`)
                }
            } catch (error) {
                console.log(error);
            }
        ...
        <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col gap-5">
                                ...
                                <FormControl>
                                    <div className='flex-center h-[54px] w-full overflow-hidden rounded-full bg-grey-50 px-4 py-2'>
                                        <Image
                                            ...
                                            height={24}
                                        />
                                        <Input type='number' placeholder="Price" {...field} className=' p-regular-16 border-0 bg-grey-50 outline-offset-0 focus:border-0 focus-visible:right-0 focus-visible:ring-offset-0 input-field' />
                ...
                type="submit"
                 ...
                 >{form.formState.isSubmitting ? ('Submittimg...'):`${type} Event` }</Button>
            </form>
        </Form>
    )
}
